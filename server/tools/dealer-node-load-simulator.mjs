...
async function sendOne(i){inFlight++;total++;const nodeNum=i%nodes;const payload={runId,nodeId:`sim-node-${String(nodeNum).padStart(6,'0')}`,companyId:`SIM${String(nodeNum).padStart(6,'0')}`,appVersion:'0.10.0',seq:++seq,payloadBytes:512};try{const r=await post('/api/load-test/heartbeat',payload);ok++;samples.push(r.ms);if(samples.length>250000)samples.shift()}catch(e){errors++;if(errors<=10)console.error('request failed:',e.message)}finally{inFlight--}}
