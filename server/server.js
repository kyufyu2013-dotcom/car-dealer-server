...
async function phase12Summary(options={}){
  ...
  return {
    serverVersion:'14.9.1',apiVersion:'6.9.1',
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'6.9.1',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'6.9.1',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'phase14d1-superadmin-live-data-center'});
  }catch(e){
...
app.post('/api/super/nodes/:companyId/request',superAuth,async(req,res,next)=>{
  try{
    ...
    if(resource==='fullCompanyData')await auditSecurityEvent(req,{action:'super_remote_live_snapshot_request',category:'super_admin_remote_read',status:'success',targetType:'company',targetId:companyId,detail:'Requested live Dealer Node full-company snapshot relay',metadata:{nodeId:n.node_id}});
...
app.get('/api/super/companies/:id/data-center',superAuth,async(req,res,next)=>{
  try{
    ...
    await auditSecurityEvent(req,{action:'super_remote_data_center_access',category:'super_admin_remote_read',status:'success',targetType:'company',targetId:companyId,detail:'Super Admin opened live dealership data center',metadata:{nodeId:node.node_id,modules:['branches','users','payroll','staff','transfers','operating_costs','settings']}});
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'14.9.1',apiVersion:'6.9.1',schemaVersion:schema.currentVersion,tables:data};
