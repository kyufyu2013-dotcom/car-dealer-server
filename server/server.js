...
async function phase12Summary(options={}){
  ...
  return {
    serverVersion:'14.9.35',apiVersion:'6.9.35',
...
app.get('/m200530366/',(req,res)=>{
  ...
  res.setHeader('X-SuperAdmin-UI-Version','14.9.35');
...
app.get('/m200530366',(req,res)=>res.redirect('/m200530366/?ui=14.9.35'));
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'6.9.35',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'6.9.35',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'multi-branch-live-operations',superAdminUiVersion:'14.9.35'});
  }catch(e){
...
app.post('/api/transfers',auth,requireActiveCompany,async(req,res,next)=>{const client=await pool.connect();try{
  ...
  const carId=String(req.body?.carId||''),requestedFromBranchId=String(req.body?.fromBranchId||''),toBranchId=String(req.body?.toBranchId||''),note=String(req.body?.note||'').trim().slice(0,500);if(!carId||!toBranchId)return res.status(400).json({error:'請選擇車輛與目的分店'});
  ...
  const c=d.cars.find(x=>String(x.id)===carId);if(!c||c.status!=='在庫'){await client.query('ROLLBACK');return res.status(409).json({error:'只有在庫車輛可以調撥'});}
  const snapshotFromBranchId=String(c.branchId||''),actorBranchId=String(actor.branch_id||''),fromBranchId=actor.role==='branchManager'?actorBranchId:String(requestedFromBranchId||snapshotFromBranchId);
  if(!fromBranchId){await client.query('ROLLBACK');return res.status(400).json({error:'找不到車輛目前所屬分店'});}
  if(actor.role==='branchManager'&&requestedFromBranchId&&requestedFromBranchId!==actorBranchId){await client.query('ROLLBACK');return res.status(403).json({error:'分店主管只能調撥自己分店的車輛'});}
  ...
  const company=(await client.query('SELECT owner_user_id FROM companies WHERE id=$1',[req.auth.companyId])).rows[0]||{},ts=now(),id='tr_'+crypto.randomUUID(),direct=actor.role==='admin',sourceReconciled=snapshotFromBranchId!==fromBranchId;
  ...
  let ver=Number(lock.rows[0].version||0);
  if(sourceReconciled&&!direct){c.branchId=fromBranchId;c.updatedAt=ts;ver+=1;await client.query('UPDATE snapshots SET version=$1,json=$2::jsonb,updated_at=$3 WHERE company_id=$4',[ver,JSON.stringify(cloudOperationalSnapshot(d)),ts,req.auth.companyId]);}
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'14.9.35',apiVersion:'6.9.35',schemaVersion:schema.currentVersion,tables:data};
