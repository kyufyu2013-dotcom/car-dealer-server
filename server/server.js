...
,
  {
    ...
    sql:`
      ...
      UPDATE payment_providers SET webhook_secret=encode(gen_random_bytes(24),'hex') WHERE webhook_enabled=TRUE AND webhook_secret='';
...
async function phase12Summary(options={}){
  ...
  return {
    serverVersion:'12.2.0',apiVersion:'4.2.0',
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'4.2.0',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'4.2.0',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'production-phase12-automated-payment-renewal'});
  }catch(e){
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'12.2.0',apiVersion:'4.2.0',schemaVersion:schema.currentVersion,tables:data};
...
app.post('/api/super/security/release-snapshot',superAuth,async(req,res,next)=>{try{const readiness=await phase10ReleaseReadiness(),releaseId=`rel_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`;await pool.query(`INSERT INTO release_control_events(release_id,server_version,api_version,schema_version,status,readiness,actor,created_at) VALUES($1,$2,$3,$4,$5,$6::jsonb,$7,$8)`,[releaseId,'12.2.0','4.2.0',readiness.schemaCurrent,readiness.status,JSON.stringify(readiness),req.auth.username||req.auth.sub,now()]);await auditSecurityEvent(req,{action:'release_readiness_snapshot',category:'release',status:'success',targetType:'release',targetId:releaseId,detail:'Rollback readiness snapshot created'});res.json({ok:true,releaseId,readiness})}catch(e){next(e)}});
...
async function applyProviderPayment(client,{provider,eventId,eventType,companyId,providerTransactionId,amountCents,currency='TWD',payload={},actor='webhook'}){
  ...
  const ts=now();
  await client.query(`INSERT INTO payment_webhook_events(provider_id,provider_event_id,event_type,status,transaction_id,payload,received_at) VALUES($1,$2,$3,'received',$4,$5::jsonb,$6)`,[provider.id,eventKey,eventType||'',providerTransactionId||'',JSON.stringify(payload||{}),ts]);
  try{
    ...
    if(['payment_failed','renewal_failed'].includes(eventType)){
      ...
      await subscriptionEvent(client,companyId,eventType,{oldStatus:sub.status,newStatus:status,oldPeriodEnd:sub.current_period_end,newPeriodEnd:sub.current_period_end,source:`provider:${provider.id}`,referenceId:eventKey,actor,detail:`自動扣款失敗，第 ${failures} 次`});
      await client.query(`UPDATE payment_webhook_events SET status='processed',processed_at=$1 WHERE provider_id=$2 AND provider_event_id=$3`,[ts,provider.id,eventKey]);return {ok:true,status,renewalFailures:failures};
    }
    if(eventType==='refund'){
    ...
    throw new Error('不支援的 Webhook eventType');
...
app.get('/api/super/subscriptions/webhooks',superAuth,async(req,res,next)=>{try{res.json({rows:(await pool.query(`SELECT w.*,p.display_name FROM payment_webhook_events w LEFT JOIN payment_providers p ON p.id=w.provider_id ORDER BY w.id DESC LIMIT 100`)).rows,renewals:(await pool.query(`SELECT r.*,c.name company_name,p.display_name provider_name FROM payment_renewal_attempts r JOIN companies c ON c.id=r.company_id LEFT JOIN payment_providers p ON p.id=r.provider_id ORDER BY r.id DESC LIMIT 100`)).rows})}catch(e){next(e)}});
...
app.post('/api/super/subscriptions/auto-renew/run',superAuth,async(req,res,next)=>{try{const r=await runAutoRenewalSweep();await auditSecurityEvent(req,{action:'auto_renewal_sweep',category:'subscription',targetType:'billing_engine',targetId:'phase12g',detail:`自動續費掃描：${r.scanned} 筆，建立 ${r.attempted} 筆`});res.json(r)}catch(e){next(e)}});
