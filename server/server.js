...
async function phase12Summary(options={}){
  ...
  return {
    serverVersion:'13.0.1',apiVersion:'5.0.1',
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'5.0.1',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'5.0.1',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'production-phase13-dealer-subscription-experience'});
  }catch(e){
...
app.post('/api/dealer/subscription-center/renewal-request',auth,async(req,res,next)=>{const client=await pool.connect();try{if(req.auth.role!=='admin')return res.status(403).json({error:'僅車行管理員可申請續費'});const access=await subscriptionAccess(req.auth.companyId,client);if(!access.managed)return res.status(400).json({error:'目前尚未綁定訂閱方案，請聯絡平台管理員'});const note=String(req.body?.note||'車行由 Desktop 送出續費需求').slice(0,500),actor=req.auth.username||req.auth.sub||'dealer_admin',ts=now();await client.query('BEGIN');const open=(await client.query(`SELECT id,status,requested_at FROM dealer_renewal_requests WHERE company_id=$1 AND status IN ('pending','contacted') ORDER BY id DESC LIMIT 1 FOR UPDATE`,[req.auth.companyId])).rows[0];if(open){await client.query('COMMIT');return res.json({ok:true,alreadyPending:true,requestId:open.id,status:open.status,message:open.status==='contacted'?'平台已收到並正在處理你的續費需求，不需要重複送出。':'續費需求已經送出，平台尚未處理，不需要重複送出。'})}const row=(await client.query(`INSERT INTO dealer_renewal_requests(company_id,plan_id,status,note,requested_by,requested_at,updated_at) VALUES($1,$2,'pending',$3,$4,$5,$5) RETURNING id`,[req.auth.companyId,access.plan_id||null,note,actor,ts])).rows[0];await subscriptionEvent(client,req.auth.companyId,'renewal_requested',{oldStatus:access.status,newStatus:access.status,oldPeriodEnd:access.periodEnd,newPeriodEnd:access.periodEnd,source:'dealer_desktop',referenceId:String(row.id),actor,detail:note});await client.query('COMMIT');res.json({ok:true,requestId:row.id,status:'pending',message:'續費需求已送出，Super Admin 已收到待處理通知。'})}catch(e){try{await client.query('ROLLBACK')}catch{};if(String(e?.code||'')==='23505')return res.json({ok:true,alreadyPending:true,message:'續費需求已經在待處理清單中，不需要重複送出。'});next(e)}finally{client.release()}});
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'13.0.1',apiVersion:'5.0.1',schemaVersion:schema.currentVersion,tables:data};
