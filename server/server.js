...
async function phase12Summary(options={}){
  ...
  return {
    serverVersion:'14.9.4',apiVersion:'6.9.4',
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'6.9.4',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'6.9.4',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'phase14d2-superadmin-overview-datacenter-ux'});
  }catch(e){
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'14.9.4',apiVersion:'6.9.4',schemaVersion:schema.currentVersion,tables:data};
