...
function cloudOperationalSnapshot(snapshot){
  ...
  for(const k of ['operationLogs','salaryHistory','payrollLocal','operatingCostLocal','serviceVehicles','serviceOrders','partsCatalog','partsInventory','partsPurchases','partsMovements'])delete d[k];
  return d;
}
...
async function phase12Summary(options={}){
  ...
  return {
    serverVersion:'14.9.36',apiVersion:'6.9.36',
...
app.get('/m200530366/',(req,res)=>{
  ...
  res.setHeader('X-SuperAdmin-UI-Version','14.9.36');
...
app.get('/m200530366',(req,res)=>res.redirect('/m200530366/?ui=14.9.36'));
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'6.9.36',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'6.9.36',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'multi-branch-live-operations',superAdminUiVersion:'14.9.36'});
  }catch(e){
...
app.post('/api/company/live-events',auth,requireActiveCompany,async(req,res,next)=>{try{
  ...
  if(!['vehicle_upsert','vehicle_delete'].includes(type)||!carId||!branchId||!eventId)return res.status(400).json({error:'即時事件資料不完整'});
  ...
  const vehicle=type==='vehicle_upsert'?sanitizeLiveVehicle(b.vehicle):null;if(type==='vehicle_upsert'&&(!vehicle||String(vehicle.id||'')!==carId||String(vehicle.branchId||'')!==branchId))return res.status(400).json({error:'即時車輛資料不完整'});
  const e={seq:++companyLiveEventSeq,eventId,type,carId,branchId,sourceNodeId:String(node.node_id),sourceRole:String(req.auth.role),occurredAt,vehicle,createdMs:Date.now()};events.push(e);companyLiveEvents.set(companyId,events.slice(-COMPANY_LIVE_EVENT_MAX));cleanCompanyLiveEvents(companyId);
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'14.9.36',apiVersion:'6.9.36',schemaVersion:schema.currentVersion,tables:data};
