...
async function phase12Summary(options={}){await refreshSubscriptionStatuses();const paymentPage=Math.max(1,Number(options.paymentPage||1)),paymentPageSize=10,paymentSearch=String(options.paymentSearch||'').trim(),paymentStatus=String(options.paymentStatus||'').trim();const wh=[],params=[];if(paymentSearch){params.push(`%${paymentSearch}%`);wh.push(`(c.name ILIKE $${params.length} OR t.description ILIKE $${params.length} OR t.id ILIKE $${params.length})`)}if(paymentStatus&&['pending','paid','failed','refunded'].includes(paymentStatus)){params.push(paymentStatus);wh.push(`t.status=$${params.length}`)}const where=wh.length?`WHERE ${wh.join(' AND ')}`:'';const paymentTotal=Number((await pool.query(`SELECT COUNT(*)::int AS n FROM payment_transactions t JOIN companies c ON c.id=t.company_id ${where}`,params)).rows[0]?.n||0),paymentTotalPages=Math.max(1,Math.ceil(paymentTotal/paymentPageSize)),safePage=Math.min(paymentPage,paymentTotalPages),offset=(safePage-1)*paymentPageSize;const payParams=[...params,paymentPageSize,offset],limitIdx=params.length+1,offsetIdx=params.length+2;const [plans,providers,subs,pays,events,companies,pendingCount]=await Promise.all([pool.query(`SELECT * FROM subscription_plans ORDER BY sort_order,name`),pool.query(`SELECT id,provider_type,display_name,enabled,mode,webhook_enabled,adapter_key,capabilities,created_at,updated_at FROM payment_providers ORDER BY CASE WHEN id='manual' THEN 0 ELSE 1 END,display_name`),pool.query(`SELECT s.*,c.name AS company_name,c.enabled,c.expires_at,p.name AS plan_name,p.price_cents,p.currency,p.billing_interval,p.billing_interval_count,p.grace_days, EXISTS(SELECT 1 FROM payment_transactions pt WHERE pt.company_id=s.company_id AND pt.status='paid') AS has_paid_payment, (SELECT MAX(pt.paid_at) FROM payment_transactions pt WHERE pt.company_id=s.company_id AND pt.status='paid') AS last_paid_at FROM dealer_subscriptions s JOIN companies c ON c.id=s.company_id LEFT JOIN subscription_plans p ON p.id=s.plan_id ORDER BY c.name`),pool.query(`SELECT t.*,c.name AS company_name,pr.display_name AS provider_name FROM payment_transactions t JOIN companies c ON c.id=t.company_id LEFT JOIN payment_providers pr ON pr.id=t.provider_id ${where} ORDER BY t.created_at DESC LIMIT $${limitIdx} OFFSET $${offsetIdx}`,payParams),pool.query(`SELECT e.*,c.name AS company_name FROM subscription_events e JOIN companies c ON c.id=e.company_id ORDER BY e.id DESC LIMIT 100`),pool.query(`SELECT id,name,enabled,expires_at,trial FROM companies ORDER BY name`),pool.query(`SELECT COUNT(*)::int AS n FROM payment_transactions WHERE status='pending'`)]);return {serverVersion:'12.0.0',apiVersion:'4.0.4',plans:plans.rows,providers:providers.rows,subscriptions:subs.rows,payments:pays.rows,paymentPagination:{page:safePage,pageSize:paymentPageSize,total:paymentTotal,totalPages:paymentTotalPages,search:paymentSearch,status:paymentStatus},pendingPaymentCount:Number(pendingCount.rows[0]?.n||0),events:events.rows,companies:companies.rows}}
...
app.get('/api/ready',async(req,res)=>{
  ...
  const body={ok:ready,time:now(),service:'car-dealer-central',version:'4.0.4',haEnabled:CENTRAL_HA_ENABLED,dbRole:st.dbRole,writeReady:ready,schemaReady:st.schemaReady,site:CENTRAL_HA_SITE,instanceId:CENTRAL_HA_INSTANCE_ID};
...
app.get('/api/health',async(req,res)=>{
  try{
    ...
    res.json({ok:true,time:now(),service:'car-dealer-central',database:'postgres',version:'4.0.4',schemaVersion:SERVER_SCHEMA_TARGET,architecture:'production-phase12-subscription-licensing'});
  }catch(e){
...
app.post('/api/auth/login',loginGuard('dealer'),async(req,res,next)=>{
  try{
    ...
    res.json({token:signUser(u,!!rememberLogin),company:{...companyDto(fresh),subscription:billing.managed?{status:billing.status,planName:billing.planName,currentPeriodEnd:billing.periodEnd,graceUntil:billing.graceUntil,autoRenew:!!billing.auto_renew}:null},user:userDto(u),snapshot:snapshotForUser(snap.snapshot,u),version:snap.version,offlineTicket:issueOfflineTicket(fresh,u,password,offlineSeconds),offlinePolicy:{graceSeconds:offlineSeconds,test:!!test?.enabled},rememberLogin:!!rememberLogin});
  }catch(e){ next(e); }
});
...
async function createCentralBackup(triggerType='manual',actor='system'){
  ...
  try{
    ...
    const payload={format:'car-dealer-central-logical-backup',formatVersion:1,createdAt:now(),serverVersion:'12.0.0',apiVersion:'4.0.4',schemaVersion:schema.currentVersion,tables:data};
...
app.post('/api/super/security/release-snapshot',superAuth,async(req,res,next)=>{try{const readiness=await phase10ReleaseReadiness(),releaseId=`rel_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`;await pool.query(`INSERT INTO release_control_events(release_id,server_version,api_version,schema_version,status,readiness,actor,created_at) VALUES($1,$2,$3,$4,$5,$6::jsonb,$7,$8)`,[releaseId,'12.0.0','4.0.4',readiness.schemaCurrent,readiness.status,JSON.stringify(readiness),req.auth.username||req.auth.sub,now()]);await auditSecurityEvent(req,{action:'release_readiness_snapshot',category:'release',status:'success',targetType:'release',targetId:releaseId,detail:'Rollback readiness snapshot created'});res.json({ok:true,releaseId,readiness})}catch(e){next(e)}});
...
app.get('/api/super/subscriptions/webhooks',superAuth,async(req,res,next)=>{try{res.json({rows:(await pool.query(`SELECT * FROM payment_webhook_events ORDER BY id DESC LIMIT 100`)).rows})}catch(e){next(e)}});
app.post('/api/payments/webhook/:providerId',async(req,res,next)=>{try{const provider=(await pool.query(`SELECT * FROM payment_providers WHERE id=$1`,[req.params.providerId])).rows[0];if(!provider||!provider.enabled)return res.status(503).json({error:'此付款通道尚未啟用'});return res.status(501).json({error:'付款 Provider Adapter 尚未安裝；Webhook 入口已預留',providerId:provider.id,adapterKey:provider.adapter_key})}catch(e){next(e)}});
